...
static TARGETS: RwLock<Vec<TargetData>> = RwLock::new(Vec::new());
...
impl Builder {
    ...
    pub fn level_str(mut self, level: &str) -> Result<Self> {
        self.level = parse_level(level)?; Ok(self)
    }

    #[inline]
    pub fn log_file_max_str(mut self, log_file_max: &str) -> Result<Self> {
        self.log_file_max = parse_size(log_file_max)?; Ok(self)
    }
}

pub fn init_log_simple(level: &str, log_file: String, log_file_max: &str, use_console: bool, use_async: bool) -> Result<()> {
...
pub fn set_level(target: String, level: log::LevelFilter) {
    let mut array = TARGETS.write();
    array.push(TargetData { target, level});
    array.sort_by(|a, b| a.target.cmp(&b.target));
}
...
impl log::Log for AsyncLogger {
    fn enabled(&self, metadata: &log::Metadata) -> bool {
        if metadata.level() <= self.level {
            ...
            for item in &*targets {
                if tn.starts_with(&item.target) {
                    return metadata.level() <= item.level;
                }
            }
            true
        } else {
